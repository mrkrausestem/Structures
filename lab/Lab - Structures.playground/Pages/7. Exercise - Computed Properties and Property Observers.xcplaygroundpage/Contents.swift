/*:
 ...
 The `Rectangle` struct below has two properties, one for width and one for height. Add a computed property that computes the area of the rectangle (i.e. width * height). Create an instance of `Rectangle` and print the `area` property. (.25 points)
 */
struct Rectangle {
    var width: Int
    var height: Int
...
/*:
 ...
 Create an instance of `Height` and then change `heightInInches`. Print out `heightInCentimeters` to ensure that it was adjusted accordingly. Then, repeat the process for `heightInCentimeters`. (.75 points)
 */
